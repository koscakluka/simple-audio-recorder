...
const AUDIO_OUTPUT_MODULE_URL = URL.createObjectURL(
  new Blob(
    [
      `
...
export default class AudioRecorder {
  ...
  createAndStartEncoder(numberOfChannels) {
    ...
    this.encoder.onstopped = () => {
      this.state = states.STOPPED;
      let mp3Blob = this.options.streaming
        ? undefined
        : new Blob(this.encodedData, { type: "audio/mpeg" });
  ...
  createOutputNode(numberOfChannels) {
    if (this.useAudioWorklet()) {
      ...
      this.outputNode = new AudioWorkletNode(
        this.audioContext,
      ...
      this.outputNode = this.audioContext.createScriptProcessor(
        4096,
        numberOfChannels,
        numberOfChannels,
      );
  ...
  async __start(paused) {
    ...
    try {
      ...
      let constraints =
        Object.keys(this.options.constraints).length > 0
